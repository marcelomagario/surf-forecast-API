...
const scrapeWebsite1 = async () => {
  ...
  try {
    ...
    const localizacao = $('.pico_header h1').text();
    const dataString = $('.pico_header h2').text();
    const data = (regexData.exec(dataString) || [null, null])[1];
    const tamanho = $('#forecast_wave_size span').text().trim();
    const direcao = $('#forecast_wave_direction').text().trim();
    const vento = $('#forecast_wind_speed').text().replace(/\s+/g, '');
